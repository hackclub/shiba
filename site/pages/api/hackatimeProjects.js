export default async function handler(req, res) {
  ...
  if (!/^[A-Za-z0-9_-]{1,50}$/.test(slackId)) {
    return res
      .status(400)
      .json({ message: "That is a funny looking slack id" });
  }

  const startDate = "2025-08-18";
  const url = `https://hackatime.hackclub.com/api/v1/users/${encodeURIComponent(
    slackId
  )}/stats?features=projects&start_date=${startDate}`;
  try {
    ...
    if (json.trust_factor) {
      ...
      if (json.trust_factor.trust_value == 1) {
        // update airtable if not done

        return res.status(403).json({
          message: `Banned from hackatime`,
          projects: [],
          projectsWithTime: [],
        });
      }
    }
    const projects = Array.isArray(json?.data?.projects)
      ? json.data.projects
      : [];
